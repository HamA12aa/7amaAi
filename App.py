...
# ==========================================
# 0. AUTO-INSTALLER (بۆ ڕێگری لە هەڵەی کتێبخانەکان)
# ==========================================
def install_requirements():
    try:
        ...
        subprocess.check_call([sys.executable, "-m", "pip", "install", "groq", "google-generativeai", "moviepy==1.0.3"])

install_requirements()

# ئینپۆرتە سەرەکییەکان
...
def extract_and_transcribe_audio(video_path, groq_key):
    try:
        ...
        return f"[سەرکەوتوو نەبوو لە دەرهێنانی دەنگ]: {str(e)}"
...
SRT Text: {srt_text[:40000]}"""
    ...
    for key in keys:
        try:
            ...
            continue
    return "❌ کێشە لە بریکاری یەکەم. تکایە دڵنیابە لە کلیلەکانی Gemini."
...
I will give you the English SRT and the actual AUDIO transcript.
Watch the video, compare the SRT with the audio and visual context.
...
*   **دەنگی ئەسڵی/ژاپۆنییەکە (Audio):** "[What the audio actually means based on the transcript]"
*   **ڕاستییەکە بەپێی دیمەن:** "[The correct Sorani translation based on video+audio]"
*   **هۆکار:** "[Why the SRT is wrong]"
---

SRT Text: {srt_text[:20000]}
Actual Audio Transcript: {audio_transcript}
"""
            ...
            continue
    return "❌ کێشە لە بریکاری دووەم لە کاتی بینینی ڤیدیۆ."

def run_agent2(keys, video_path, srt_text, audio_transcript):
    part1, part2 = process_video(video_path)
    
    if not part1:
        return "❌ هەڵەیەک ڕوویدا لە کاتی بچووککردنەوەی ڤیدیۆکە."
...
with col1: 
    srt_input = st.text_area("📄 دەقی ئینگلیزی (SRT) دابنێ:", height=200)
with col2: 
    video_input = st.file_uploader("🎥 ڤیدیۆکە دابنێ:", type=["mp4", "mkv"])

if st.button("🚀 دەستپێکردنی ستۆدیۆ (کارکردنی هاوکات)"):
    if not active_g_keys:
        st.error("تکایە کلیلی Gemini دابنێ!")
    elif not groq_key:
        st.error("تکایە کلیلی Groq دابنێ!")
    elif not srt_input:
        st.error("تکایە دەقی ژێرنووسەکە دابنێ!")
    elif not video_input:
        st.error("تکایە ڤیدیۆکە دابنێ!")
    else:
        ...
        with tab1:
            ...
            st.download_button("📥 داگرتنی شیکاری چیرۆک (TXT)", result_agent1, "Agent1_Story_Bible.txt", key="dl_a1")
        ...
        with tab2:
            ...
            st.download_button("📥 داگرتنی هەڵەکانی ڤیدیۆ (TXT)", result_agent2, "Agent2_Visual_Errors.txt", key="dl_a2")
