...
def generate_content_with_retry(prompt, active_keys, model_name, temp=0.2):
    """ئەمە نهێنییەکەیە: تا کوردییەکە نەرداتەوە کۆڵ نادات!"""
    max_attempts = 8 # ٨ جار هەوڵ دەدات ئەگەر ئێرۆر هەبێت
    ...
    for attempt in range(max_attempts):
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                time.sleep(4) # ئەگەر خێرا بووین، ٤ چرکە دەوەستێت
            else:
                time.sleep(2)
        except Exception as e:
            time.sleep(2)
            
    return None # تەنها ئەگەر هەموو هەوڵەکان شکستیان هێنا
...
Output ONLY valid XML for every single line like this:
<sub id="1">
...
3. Output ONLY XML format: <sub id="X">Perfect Kurdish Text</sub>
Input:
{input_text}"""
...
def process_swarm_pipeline(chunk_15, active_keys, model_name, split_limit, temp, master_context):
    ...
    if not enriched: return None, {}, f"⚠️ بریکاری ٢ شکستی هێنا (لیمیتە یان هێڵی ئینتەرنێت)."
    ...
    if not final_res: return None, enriched, f"⚠️ بریکاری ٣ شکستی هێنا. بەکارهێنانی پێشنیاری بریکاری ٢."
...
# 5. MAIN UI & SIDEBAR
# ==========================================
st.markdown("<h1 class='main-title'>AI Movie Studio PRO 🎬 V19</h1>", unsafe_allow_html=True)
...
with tab1:
    ...
    if input_srt and active_keys:
        ...
        st.markdown(f"<div class='eta-box'><b>⏳ کاتی پێشبینیکراو بۆ {lines_count} دێڕ:</b> نزیکەی {int(2 + (lines_count/15/len(active_keys))*2)} خولەک</div>", unsafe_allow_html=True)
...
if st.session_state.is_running:
    with tab2:
        ...
        if chunks_to_process:
            ...
            # ڕێگریکردن لە کراشکردنی سێرڤەر بە بەکارهێنانی زۆرترین 3 Thread
            max_threads = min(3, len(active_keys)) 
            
            with concurrent.futures.ThreadPoolExecutor(max_workers=max_threads) as executor:
                future_to_chunk = {}
                for idx, chunk_data in enumerate(chunks_to_process):
                    i, chunk = chunk_data
                    log_to_console(f"ناردنی پارچەی {i+1} بۆ بریکارەکان...", "aqua")
                    update_console()
                ...
                for future in concurrent.futures.as_completed(future_to_chunk):
                    ...
                    for item in chunk:
                        ...
                        # ئەگەر هیچی نەبوو، ڕێک دەبێت دووبارە هەوڵ بداتەوە، بەڵام ئەمە ڕوونادات چونکە loopـەکەمان ٨ جارە
...
with tab3:
    if "final_srt" in st.session_state and st.session_state.final_srt:
        st.balloons()
        c1, c2 = st.columns(2)
        with c1: st.download_button("📥 داگرتنی فایلی تایتان (.srt)", st.session_state.final_srt, "Studio_PRO_V19.srt")
        with c2: st.download_button("📝 داگرتنی وەک دەقی سادە (.txt)", st.session_state.final_srt.replace('-->', '>>'), "Studio_PRO.txt")
